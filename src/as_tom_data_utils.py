...
def apply_categorical_mapping(pdseries, categories_dict):
    if "other" not in categories_dict.keys():
        ...
        if isinstance(test_value, str):
            ...
            raise ValueError(
                "Categorical mappings only support boolean or string arguments"
            )
...
def get_risk_factors(df):
    ...
    df["BloodPressure"] = apply_categorical_mapping(
        df["Risk factors"], {"high bp": True}
    )
    df["HighCholesterol"] = apply_categorical_mapping(
        df["Risk factors"], {"high cholesterol": True}
    )
...
def compute_intervals(df, unit, quantity, if_offset=False):
    ...
    df["leftover_frames"] = (
        df["frames"] - df["num_intervals"] * df["window_size"]
    ).astype("int32")

    video_idx, interval_idx, start_frame, end_frame, label, study_id = (
        [],
        [],
        [],
        [],
    ...
    for i in range(len(df)):
        ...
        if video_info["num_intervals"] == 0:
            ...
            label.append(video_info["as_label"])
            study_id.append(video_info["Echo ID#"])
        else:
