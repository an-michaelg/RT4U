...
def sample_classbags_balance(arr, batch_size=4):
    ...
    # # sort the dict in ascending order of keys (classes from 0..K)
    # # determine the odds of sampling each class
    # counts = []
    # for c, inds in sorted(indices_of_class.items()):
    ...
    sample_of_classes = np.random.choice(n_classes, n_batches)
    batches_list = []
...
class MILSampler(Sampler[List[int]]):
    def __init__(
        self, study_id, as_label, batch_size=4, groupby_study=False, balance_label=False
    ) -> None:
        ...
        # determine number of batches per epoch via a preliminary run of the selected method
        if self.groupby_study == False:
            if self.balance_label == False:
                # balanced sampling wo replacement BUT each batch shares a label class
                batches = sample_classbags_wo_replacement(
                    self.as_label, self.batch_size
                )
            ...
            if self.balance_label == False:
                ...
                # label-balanced sampling, each batch represents a single study
                batches = sample_studies_balance(
                    self.study_id, self.as_label, self.batch_size
                )
...
if __name__ == "__main__":
    ...
    sampler = MILSampler(
        study, arr, batch_size, groupby_study=False, balance_label=True
    )
